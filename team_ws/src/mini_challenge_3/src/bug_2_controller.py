...
class Bug0():
    ...
    def get_line_to_goal(self):
        xgoal, ygoal = self.target_postition_xy_2d[0], self.target_postition_xy_2d[1]
        xbot, ybot = self.current_position_xy_2d[0], self.current_position_xy_2d[1]
        dy = ygoal - ybot
        dx = xgoal - xbot
        self.a = dy
        self.b = -dx
        self.c = dx*ygoal + dy*xgoal
        self.state = "go_to_point"
        self.best_to_goal()


    def best_to_goal(self):
        euclidian_distance = nav_functions.euclidean_distance_point_to_point_2d(self.current_position_xy_2d,self.target_postition_xy_2d)
        if euclidian_distance <= self.best_hit:
            self.best_hit = euclidian_distance
            return True
        return False
    ...
    def current_pos_in_line(self):
        xbot, ybot = self.current_position_xy_2d[0], self.current_position_xy_2d[1]
        bool_ans = self.a*xbot + self.b*ybot + self.c < self.line_treshold
        return bool_ans
    ...
    def turn_left(self, angle_to_rotate = np.pi/2.0):
    ...
    def right_hand_rule_controller(self):
        ...
        if self.scan != None:                       
            if self.current_pos_in_line() and self.best_to_goal and (rospy.get_time() - self.follow_wall_start_time) >= 1.0:
                print ("state changed")
                self.state = "go_to_point"
            elif self.obstacle_in_front():
                self.state = "turn_left"                
